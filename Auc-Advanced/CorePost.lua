--[[
	Auctioneer
	Version: 8.0.6122 (SwimmingSeadragon)
	Revision: $Id: CorePost.lua 6122 2018-09-09 15:10:05Z none $
...
function lib.GetDepositCost(item, duration, bidprice, buyprice, stacksize, numstacks)
	...
	### From WoW 8.0 Deposit Cost update this rounding is no longer correct
	### Currently set to round down to nearest whole, until new rounding algorithm is determined
	--]]

	-- Set up function defaults if not specifically provided
	duration = depositDurationMultiplier[duration] or 2
	if not stacksize or stacksize < 1 then stacksize = 1 end

	local gsv, classID, subclassID = AucAdvanced.GetItemInfoCache(item, 11)
	if not gsv then
	...
	if gsv then
		local deposit = floor(0.15 * gsv * stacksize * duration)
		--### todo: need to determine proper rounding algorithm

		if classID == LE_ITEM_CLASS_TRADEGOODS then
			-- additional deposit fee for certain tradegoods
			if subclassID ~= 16 and subclassID ~= 10 and subclassID ~= 4 and subclassID ~= 11 and subclassID ~= 1 then -- exclude certain subclasses
				local extra = max(bidprice or 1, buyprice or 0)
				extra = extra / stacksize

				deposit = deposit + floor(extra * .2) -- extra fee is not affected by auction duration
				--### todo: need to determine proper rounding algorithm
			end
		end
...
AucAdvanced.RegisterRevision("$URL: Auc-Advanced/CorePost.lua $", "$Rev: 6122 $")
